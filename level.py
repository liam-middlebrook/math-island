...
def loadimage(fn):
    ...
    For now, stubbed out to just give the filename
    '''
    return fn
...
class BlockingTile(object):
    def __init__(self, fn=None):
        self.image = loadimage(fn)
        self.passable = False

class WaterTile(BlockingTile):
    def __init__(self):
        BlockingTile.__init__(self, path.join(CONTENTHOME, "water.png"))

class Tiles(object):
    def __init__(self):
        assert path.exists(path.join(LEVELHOME, "tiles")), "Could not load tiles"
        self.tilemap = {}
        with open(path.join(LEVELHOME, "tiles")) as tiledata:
